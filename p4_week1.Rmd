...
<span style="color: red;"> Additional Ideas but I do not know whether we have time for them: </span>  
...
<span style="color: red;">Add section discussion FAIR data and referring to several platforms where it can be found? Several reserach consortia provide easy access and clear description of their data through data portals. These files can be downloaded or immediately uploaded to the UCSC Genome browser using the URL. </span> 
...
![Top menu bar in UCSC Genome Browser](images/week1/3b_ucsc_gb_looking_at_genes/ucsc_genome_browser_fig1_startpage_topmenubar.png){width=100%}   
...
![Genomes page, ready to go to *ACTB* gene in Human genome GRCh38 ](images/week1/3b_ucsc_gb_looking_at_genes/ucsc_gb_actb_question_startpage.png)  
...
<br>
> UCSC GB session for making following exercises (tip: use right-mouse click to not loose this tutorial page): Default should look like (if it doesn't, I need to adjust the text): http://genome-euro.ucsc.edu/s/ctoenhake/hg38_default    
...
<br>
<span style="color: red;"> Question asign what the name of the track with gene locations is or picutre with c/b/c/d/e/f etc. instead of track name and asking which one diplays the chromosomal coordinates and which one the gene locations?</span> 
...
<br>
> UCSC GB sessions for making following exercises (tip: use right-mouse click to not loose this tutorial page): Genes and ruler only: http://genome-euro.ucsc.edu/s/ctoenhake/hg38_genesonly   

<br>
More information about *ACTB* can be obtained by clicking on the gene in the browser:  
![(Top part of the gene information page *ACTB*) ](images/week1/3b_ucsc_gb_looking_at_genes/ucsc_gb_actb_geneinformation.png){width=100%}\    
<br>
...
<span style="color: red;">Are up- & downstream discussed?<\span>  
...
<br>
> UCSC GB sessions for making the following exercises (tip: use right-mouse click to not loose this tutorial page):  Monocyte wiggle data (blueprint in colors, encode in black): http://genome-euro.ucsc.edu/s/ctoenhake/hg38_monos_blueprint_and_encode    
...
<span style="color: red"> or is the promoter spanning 3kb and including the H3K4me1 peak? </span>  
...
<br>
<span style="color: orange; font-family: Arial; font-size:18px;">ACTION: Zoom out 3x and then another 10x.</span>     
...
> UCSC GB session for the following exercises (tip: use right-mouse click to not loose this tutorial page): Monocyte wiggle + peak data, blueprint only: http://genome-euro.ucsc.edu/s/ctoenhake/hg38_monos_blueprint_and_encode/hg38_monos_blueprint_wiglle_and_peaks  

<br>
```{r Q10_FSCN1_bivalent_promoter, eval = F, echo=T, exercise = T}
...
<br>
```{r Q11_FSCN1_expression, eval = F, echo=T, exercise = T}
...
<br>
```{r Q12_ACTB_enhacer_one, eval = F, echo=T, exercise = T}
...
```
<br>
```{r Q13_ACTB_enhancer_two, eval = F, echo=T, exercise = T}
question("Question 13: What functional element do these marks most likely reflect?",
  answer("terminator"),
  answer("promoter"),
  answer("TSS"),
...
<br>
```{r Q14_cd14_questions, eval = F, echo=T, exercise = T}
question("Question 14: What statement about *CD14* is correct? (Select ALL that apply) ",
...
```{r Q15_cd14_enhancer, eval = F, echo=T, exercise = T}
question("Question 15: Which marks mark the enhancer 20kb downstream of *CD14*?",
...
```{r cd14_near_silencing, eval = F, echo=T, exercise = T}
...
```
<br>
```{r cd14_near_silencing_two, eval = F, echo=T, exercise = T}
question("Question 14: What is the approximate size of the H3K9me3 silenced region? (*Tip2: click-and-drag in the brwoser to capture and thereby measure the width of the H3K9me3 domain)",
  answer("250kb"),
  answer("500kb"),
...
<br>
```{r ACTA1_marking_one, eval = F, echo=T, exercise = T}
...
```
<br>
```{r ACTA1_marking_two, eval = F, echo=T, exercise = T}
...
### 1.4.6 Summary  
To summarize, we have seen various marks, finishe the following statements based on your observations in the UCSC genome browser:  
