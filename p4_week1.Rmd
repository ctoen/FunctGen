...
> #### Learning Objectives  
1. Know how raw reads (from eg ChIP-seq) are processed to obtain trustworthy data.  
...
### 1.2 From raw data to science   
![](images/week1/whatbioinformaticiancsdo.png)
<br>

The sequencer determines the sequnce of the  DNA molecules in your sample. The DNA sequences are saved in files. There are two data formats to save DNA sequuences: `.fastq` and `.fasta`:  
<br>

fasta:  
![](images/week1/sample_fasta.png)
- line 1 has the sequence id, name, sometimes location. Elements seperated by `|` or space  
- line 2 contains the DNA sequence. Sometimes split over multiple lines.   
fastq: 
![](images/week1/sample_fastq.png)
4 lines per DNA sequence  
 | 
line 1 has the location/id and starts with `>` | line 1 has the sequence ID given by the sequencer and starts with a '#'  
