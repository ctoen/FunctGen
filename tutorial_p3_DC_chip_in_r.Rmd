...
`dba.plotBox()` returns several insights:  

- boxplot showing the log2 normalized reads in binding sites present in both samples but also for sites with increased reads in either of the conditions, so where differential binding occurs.  
- p-values for the pairwise comparisions between these two groups  
...
Here: what does it mean that this set of peaks is more pronounced in one sample compared to the other? How can we use this to understand the different phenotypes and the different responses to treatment?  
Remember, what wer are eventually actually interested in  are the effects on gene regulation.  

Which gene is affected by a given binding site?  
- Impossible to know for sure
- Commonly used heuristic is assignmetn to closest gene  

#### Acquire gene location information  
For this need information about gene location. --> From annotation packages in `TxDb` packages.    

- uses entrez geneIDs  

Aditional packages to translate these IDs to perhaps more usefull names, eg gene sympols: `org.Hs.eg.db`  
`select(org.Hs.eg.db, keys = gene_id, columns = "SYMBOL", keytype = "ENTREZID")`  

But as some genes have mutliple symbols. Is is advised to work as long as possible with the unque entrezIDs.  

#### Annotating peaks  
`ChIPpeakAnno` package  
`annoPeaks(peaks, human_genes, bindingType = "StartSizte", bindingRegion = c(-5000,5000))`  

- 2 GRanges objects: peak coordinates and annotations.  

- match them by other criteria's: bindingtype and bindingregion. bindingRegion indicates what region around the bindingtype, the peak should fall in to  

#### Visualizing similarities and differences  
Venndiagram from `DiffBind` pckage  

`dba.plotVenn(peaks, mask = 1:2)`  --- only for small sample size  

Use `UpSet` package for multiple sample comparison  
`called_peaks <- as.data.frame(peaks$called)`   
`upset(called_peaks, sets = colnames(peaks$called), order.by = 'freq')`  

Need dataframe with indicated for each peak in which samples it was present. DBA object gives this in the "called" object.  

## 4.2 Code: Consolidating peaks   
How to obtain a merged peak set that incorporates calls from all samples..   

`ChIPpeakAnno` package --> `findOverlapsOfPeaks()` function can merge peak sets as GRagnes from up to five samples  

1. Extract peaks from the ChIPQCexperiment object: `peak_calls <- peaks(ar_calls)` using `peaks()` argument, returns GRangesList object.    
2. Discard alls amples that failed QC  `peak_passed <- peak_calls[qc_pass]` -- don't know where `qc_pass` comes from. It selects 4 samples from the original list of 9.  
3. Find overlaps between peak sets using `findOverlapsOfPeaks()` function  `peaks_combined <- findOverlapsOfPeaks(peak_passed[[1]], peak_passed[[2]], peak_passed[[3]], peak_passed[[4]], maxgap=50)`  
4. Examine the merged peak set `mergedPeaks`  

In mergedPeaks have first element with venn counts.  

## 4.3 Code: Using annotations   
`TxDb.Hsapiens.UCSC.hg19.knownGene` package provides the coordinates of all known human genes.  

`org.Hs.eg.db` package provides mappings between different sets of gene identifiers.   

1. Obtain gene symbols from the SYMBOL column of org.Hs.eg.db. `gene_symbol <- select(org.Hs.eg.db, keys=human_genes$gene_id, columns="SYMBOL", keytype="ENTREZID")`   

2. Examine the structure of the returned annotations.  
`str(gene_symbol)`  

3. Add gene symbols to human_genes.  
`human_genes$symbol <- gene_symbol`  

4. Examine the result.  
`print(human_genes)`  

## 4.4 Code: Annotating peaks   
Combine information from the `human_genes` table and the `peaks_merged` table using `annoPeaks()` function. --> returns data frame per gene in question inclduign a column `insideFeature` indicated where the peak is located realtive to the gene.  


1. Annotate peaks with the closest gene.  
`peak_anno <- annoPeaks(peaks_merged, human_genes, bindingType="startSite", bindingRegion=c(-5000,5000))`  
2. Determine the number of peaks that were annotated with genes.  
`length(peak_anno)`  
3. Create a summary table indicating where peaks were located relative to genes.   
`table(peak_anno$insideFeature)`  

Most peaks are inside gene.  

## 4.5 Code: Which peaks are different?   
Comparing samples in venndiagram:  `dba.plotVenn(ar_diff, mask = 1:4)`  
<br>
The `upset()` function requires you to provide a data frame with one column for each set, i.e. sample, indicating set membership for each peak. The required data is contained in the `called` field of the `ar_diff` object.  
...
From gene lists to molecular mechanisms  

--> use gene set enrichment approach. How manyu of the genes in each set are associated with peaks of interests. Genes set with high proportion are likely to be of particular relevance   

package: `chipenrich`  

- annotatioin happens automatically during enrichment analysis  
- `chipenrich(peaks, genome = 'hg19', genesets = 'hallmark', locusdef = 'nearest')`  

  * genesets = to select one of several supported gene sets 
  * locusdef = how to genes should be associated with genes, eg closest TSS but can also use windows of varying size  

## 4.7 Code: Associating peaks with genes   
Explore how peak locations relate to to genes 
<br>
`plot_dist_to_tss()` function from `chipenrich` package.  Creates plot of the distribution of distances between peak locations and the TSS of genes. 
<br>
`plot_dist_to_tss(peaks, genome = "hg19")` --> barplot with proportion of peaks. Most peaks occur 10-50 kb from the TSS  
<br>
`plot_chipenrich_spline()` gives the frequency of peaks relative to the length of genes.  In addition to the observed frequencies, it also plots the expected distribution for Fisher's exact test, a binomial model, and the model used by the `chipenrich()` function.  
<br>
...
`chipenrich()` function: finding groups of genes that are more frequently associated with ChIPseq peaks than would be expected by chance.  
<br>
need to determine how genes are grouped together, use **Hallmark genesets** from Broad Institute [ref](https://www.gsea-msigdb.org/gsea/msigdb/collections.jsp)  
<br>
Usually only look at the differentially bound peaks. But in this case, only chr20 and thus small dataset, look at peaks with stronger signal in resistant sample.  
<br>

1. Select all peaks with higher intensity in treatment resistant samples  
...
`enrich_turp <- chipenrich(peaks_comb[turp_peaks, ], genome="hg19", genesets = "hallmark", out_name = NULL, locusdef = "nearest_tss", qc_plots=FALSE)`  

3. Print the results of the analysis  
`print(enrich_turp$results)`  --> sort by p-value to find most relevant gene sets  

## 4.9 Code: Understanding the impact on pathways   
## 4.10 Code A closer look at pathways  
Create a URL that links to the KEGG pathway that came up in the analysis.  
General format of this URL is:`https://www.kegg.jp/pathway/pathway/<pathway_id>+<gene_id>+...+<gene_id>`
<br>

1. create the base url:  
`base_url <- "https://www.kegg.jp/pathway/"`  
2. Add pathway ID to URL:  
`path_url <- paste0(base_url, top_path)`  
3. Collapse gene IDs into selection string  
