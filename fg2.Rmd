...
functionsfile <- paste("data", "functions.r", sep = "/")
...
rdata <- paste("data", "week2.Rdata", sep = "/")
load(rdata)
...
promoters_chr19 <- resize(promoters_chr19, width = 2200, fix = "start")
...
rdsfile <- paste("data", "prepared_rds", "blueprint_c000s5_gene_quantification_chr19.rds", sep = "/")
quantification_chr19 <- readRDS(rdsfile)


# gene annotation for annotateWithGeneParts
gencode_chr19 <- readTranscriptFeatures("data/encode/gencodev32.chr19.bed", unique.prom = TRUE, up.flank = 2000, down.flank = 200) 
```

```{r timeout1, context="server", eval = F, echo = F}
isolate({
...
```{r examplecode, exercise=TRUE, exercise.eval = F, error = T, eval = F, echo = T}
...
```{r examplecode-solution, eval = T, echo = T}
# solution
...
```{r q1_head_h3k4me3-solution, eval = T, echo = T}
# solution
head(monocytes_h3k4me3)
```

```{r q1_head_h3k4me3-check, eval = F, echo = T}
grade_result(
 pass_if(~identical(.result, head(monocytes_h3k4me3)))
 )
```

```{r quiz_grangesobject, echo = T, eval = F}
quiz(caption = "", 
...
```{r q2a_exploreh3k4me3, exercise=TRUE, exercise.eval = F, error = T, eval = F, echo = T}
...
```{r q2a_exploreh3k4me3-solution, eval = T, echo = T}
# solution
...
```{r q2a_exploreh3k4me1, exercise = T, eval = F, echo = T}
# And how many H3K4me1 peaks do you have?


```

```{r q2a_exploreh3k4me1-solution, eval = T, echo = T}
# solution
...
```{r q2b_widthh3k4me3, exercise=TRUE, exercise.eval = F, error = T, eval = F, echo = T}
...
```{r q2b_widthh3k4me3-solution, eval = T, echo = T}
# solution
...
```{r q2b_widthh3k4me1, exercise=TRUE, exercise.eval = F, error = T, eval = F, echo = T}
# And what is the distribution of H3K4me1 peak sizes?   
monocytes_h3k4me1
```

```{r q2b_widthh3k4me1-solution, eval = T, echo = T}
# solution
...
```{r quiz_peakgranges, echo = T, eval = F}
quiz(caption = "", 
...
>  
> * All chip-seq peaks are concatenated in the GRanges object `monocytes_all`.   
...
```{r monocytesall, exercise = TRUE, exercise.eval = T}
...
```{r monocytesall-solution, eval = T, echo = T}
# solution
show(monocytes_all)
```
...
```{r barplotvariable, echo = T, eval = F}
...
```{r q_extractmetadata, exercise=TRUE, error = T, eval = F, echo = T, exercise.eval = F}
# extract metadata 
...
```{r q_extractmetadata-solution, eval = T, echo = T}
# solution
# extract metadata 
monocytes_metadata <- mcols(monocytes_all)
...
```{r q_extractmetadata-code-check, eval = F, echo = T}
grade_code()
```
...
```{r barplotpeakcount, exercise=TRUE, exercise.eval = F, error = T, eval = F, echo = T}
...
```{r barplotpeakcount-solution, eval = T, echo = T}
# solution
# plot the number of peaks per chip
ggplot(data = monocytes_metadata)+
  geom_bar(mapping=aes(y=chip))+
...
```{r q_barplotpeakcount-code-check, eval = F, echo = T}
grade_code()
```
...
```{r addpeaksize, exercise = T, echo = T, eval = F}
...
```{r addpeaksize-solution, eval = T, echo = T}
# solution
...
```{r plotpeaksize, exercise=TRUE, exercise.eval = F, error = T, eval = F, echo = T}
...
```{r plotpeaksize-solution, eval = T, echo = T}
# solution
ggplot(data = monocytes_metadata)+
...
```{r plotpeaksizelog, exercise=TRUE, exercise.eval = F, error = T, eval = F, echo = T}
...
```{r plotpeaksizelog-solution, eval = T, echo = T}
# solution
ggplot(data = monocytes_metadata)+
  geom_boxplot(mapping = aes(x = chip, y = peak_size), fill = "gray", col = "black")+
  theme_bw()+
...
```{r q_broad_vs_narrow, echo = T, eval = F}
...
```{r summarizewidth, exercise = TRUE, error = T, eval = F, echo = T, exercise.eval = F}
...
```{r summarizewidth-solution, eval = T, echo = T}
# solution
monocytes_metadata %>% 
  group_by(chip) %>% 
...
```{r summarizewidth-code-check, eval = F, echo = T}
grade_code()
```
...
```{r plotsummarizewidth, exercise=TRUE, exercise.eval = F, error = T, eval = F, echo = T}
...
```{r plotsummarizewidth-solution, eval = T, echo = T}
# solution
monocytes_metadata %>%  group_by(chip) %>% 
  summarise(total_bps = sum(peak_size),  
            fraction_chr19 = sum(peak_size) /58617616) %>% 
...
```{r plotsummarizewidth-code-check, eval = F, echo = T}
grade_code()
```

```{r mcsummarizedwidth, echo = T, eval = F}
...
```{r retrieve_genes_txdb, exercise = T, echo = T, eval = F}
# load the txdb package which holds transcript-based gene models of hg38 genome  
library(TxDb.Hsapiens.UCSC.hg38.knownGene)
txdb <- TxDb.Hsapiens.UCSC.hg38.knownGene 
...
```{r q6a_proms, exercise = T, echo = T, eval = F}
...
```{r q6a_proms-solution, eval = T, echo = T}
length(promoters_chr19)
```

```{r q6a_proms-check, eval = F, echo = T}
grade_result(
...
```{r q6b_proms, exercise = T, echo = T, eval = F}
...
```{r q6b_proms-check, eval = F, echo = T}
grade_result(
...
```{r q6c_overlap, exercise=TRUE, exercise.eval = F, error = T, eval = F, echo = T}
...
```{r q6c_overlap-solution, eval = T, echo = T}
# Find overlap between monocytes_h3k4me3 peaks and promoters
...
```{r q6c_overlap-code-check, eval = F, echo = T}
grade_code()
```
<br>
...
```{r q6d_overlap, exercise=TRUE, exercise.eval = F, error = T, eval = F, echo = T, exercise.setup = "prepare_exercise6"}
...
```{r q6d_overlap-solution, eval = T, echo = T}
# Extract the unique promoter indexes
...
```{r q6d_overlap_textbox, echo = T, eval = F}
question_text(
...
overlap <- findOverlaps(query = monocytes_h3k4me3, subject = gencode_chr19$promoters)
unique_promoters <- unique(subjectHits(overlap))
unique_promoters_count <- length(unique_promoters)
```

```{r q6d_overlap_enterfraction, exercise=TRUE, exercise.eval = F, error = T, eval = F, echo = T, exercise.setup = "prepare_exercise6d"}
# enter the fraction of promoters that are part of the overlap, round to 3 decimal places:  


```

```{r q6d_overlap_enterfraction-solution, eval = T, echo = T}
# solution
round(unique_promoters_count/length(promoters_chr19),3)
```

```{r q6d_overlap_enterfraction-check, eval = F, echo = T}
grade_result(
...
```{r q6e_overlap, exercise=TRUE, exercise.eval = F, error = T, eval = F, echo = T, exercise.setup = "prepare_exercise6"}
...
```{r q6e_overlap-solution, eval = T, echo = T}
# solution
# Extract the unique peaks
...
```{r q6e_overlap_textbox, echo = T, eval = F}
question_text(
...
```{r q6e_overlap_enterfraction, exercise=TRUE, exercise.eval = F, error = T, eval = F, echo = T, exercise.setup =  "prepare_exercise6e"}
# enter the fraction of promoters that are part of the overlap, round to 3 decimal places:  


```

```{r q6e_overlap_enterfraction-solution, eval = T, echo = T}
# solution
round(unique_peaks_count/length(monocytes_h3k4me3),3)
```

```{r q6e_overlap_enterfraction-check, eval = F, echo = T}
grade_result(
...
```{r q6f_plotvenn, exercise = TRUE, exercise.eval= FALSE, error = T, eval = F, echo = T, exercise.setup = "prepare_exercise6f"}
...
```{r q6f_plotvenn-solution, eval = T, echo = T}
# solution
# determine the number of common counts
common_counts <- min(unique_promoters_count, unique_peaks_count)
...
```{r q7b_plotvenn-code-check, eval = F, echo = T}
grade_code()
```
...
```{r q7a_promfraction, exercise=TRUE, exercise.eval = F, error = T, eval = F, echo = T, message = F}
...
```{r q7a_promfraction-solution, eval = T, echo = T}
# solution
# define the variable 'promoter_size'
...
```{r q7a_promfraction_enterfraction, exercise = T, echo = T, eval = F}
# enter the fraction of promoters in chromosome 19, round of to 3 decimals


```

```{r q7a_promfraction_enterfraction-solution, eval = T, echo = T}
# solution
round(0.06179487	,3)
```

```{r q7a_promfraction_enterfraction-check, eval = F, echo = T}
grade_result(
...
<br>
```{r q7b_binomtest, exercise = TRUE, exercise.eval = F, error = T, eval = F, echo = T, exercise.setup = "prepare_q7b"}
...
```{r q7b_binomtest-solution, eval = T, echo = T}
# solution
# Use the indexes from the overlap object to identify promoters with H3K4me3 peak
...
```{r q7c_enrichmenttest, echo = T, eval = F}
...
```{r q8a_genomationoverlap, exercise = TRUE, exercise.eval = F, error = T, eval = F, echo = T}
...
```{r q8b_genomationoverlap, exercise = TRUE, exercise.lines = 3, exercise.eval = F, error = T, eval = F, echo = T, exercise.setup = "readin_gencode"}
...
```{r q8c, exercise = TRUE, exercise.lines = 6, echo = T, eval = F}
...
```{r q9a_genesids_overlap, exercise = TRUE, exercise.eval = F, error = T, eval = F, echo = T, exercise.setup = "prepare_exercis9"}
...
```{r q9a_genesids_overlap-solution, eval = T, echo = T}
# solution
# the objects `overlap` and `promoters_chr19` are loaded for you 
# subset for promoters that are part of the overlap with H3K4me3 in monocytes 
...
```{r q9a_genesids_overlap-check, eval = F, echo = T}
grade_result(
...
> **Exercise 10a**: first use dplyr::mutate(NEW_VARIABLE = ....) to define a new column `h3k4me3_promoter` which groups genes based on their overlap (TRUE/FALSE) with H3K4me3 peaks.  
...
```{r q9a_geneexpression, exercise = TRUE, exercise.eval = F, error = T, eval = F, echo = T, exercise.setup = "prepare_exercise10a"}
...
```{r q9a_geneexpression-solution, eval = T, echo = T}
# solution
# The object quantification_chr19 holds the RNA-seq quantification for genes on chromosome 19. 
# View the structure to identify the variable you will use in mutate. 
str(quantification_chr19)
...
```{r q9a_geneexpression-code-check, eval = F, echo = T}
grade_code()
```
...
```{r q10b_plot_genequant, exercise =  TRUE, exercise.eval = F, error = T, eval = F, echo = T, exercise.setup = "prepare_exercise10b"}
...
```{r q10b_plot_genequant-solution, eval = T, echo = T}
# solution
...
```{r q10b_plot_genequant-code-check, eval = F, echo = T}
grade_code()
```
...
```{r q10c_plot_genequant, exercise =  TRUE, exercise.eval = F, error = T, eval = F, echo = T, exercise.setup = "prepare_exercise10b"}
...
```{r q10c_plot_genequant-solution, eval = T, echo = T}
# solution
# Repeat the plot of 9b but lgo10-transform the y-axis    
ggplot(quantification_chr19_new)+
  geom_boxplot(mapping = aes(x = h3k4me3_promoter, y = (FPKM+1)))+
...
```{r q10c_plot_genequant-code-check, eval = F, echo = T}
grade_code()
```

There is a clear difference in gene expression between the two gene sets with presence of H3K4me3 being associated with higher expression. However,  H3K4me3 peak presence clearly does not explain everything.  
<br>  
...
```{r q11_H3K27me3_prom, exercise =  TRUE , exercise.eval = F, error = T, eval = F, echo = T}
...
```{r q11_H3K27me3_prom-solution, eval = T, echo = T}
# solution
# 1. identify the overlap between monocytes_h3k27me3 and promoters_chr19
...
```{r q11_H3K27me3_prom-code-check, eval = F, echo = T}
grade_code()
```
...
```{r q12a_bivalent_prom, exercise = TRUE, exercise.eval = F, error = T, eval = F, echo = T, exercise.setup = "prepare_exercise12"}
...
```{r q12a_bivalent_prom-solution, eval = T, echo = T}
# solution
...
```{r q12a_bivalent_prom-check, eval = F, echo = T}
grade_result(
...
```{r q12b_bivalent_prom, exercise =  TRUE, exercise.eval = F, error = T, eval = F, echo = T, exercise.setup = "prepare_exercise11b"}
...
```{r q12b_bivalent_prom-solution, eval = T, echo = T}
# solution
ggplot(quantification_chr19_new) +
...
```{r q12_bivalent_mc, echo = T, eval = F}
...
```{r q13a_upsetrplot_mark_overlap, exercise = TRUE, error = T, eval = F, echo = T, exercise.eval = F, exercise.setup =  "prepare_exercise13a"}
...
```{r q13a_upsetrplot_mark_overlap-solution, eval = T, echo = T}
# solution
# What are the names of the sets?
...
```{r q13_bivalent_mc, echo = T, eval = F}
quiz(caption ="", 
...
> **Exercise 13b**: Filter `monocytes_combinationmatrix` for combinations of 100kb and more. And plot the filtered combination matrix with `UpSet()`.   


```{r q13b_upsetrplot, exercise = TRUE, exercise.setup = "prepare_exercise13a", exercise.eval = F, error = T, eval = F, echo = T}
...
```{r q12b_upsetrplot-solution, eval = T, echo = T}
# solution
...
```{r q13b_upsetrplot-code-check, eval = F, echo = T}
grade_code()
```
...
```{r q13c_niceupset, exercise = TRUE, exercise.setup = "prepare_exercise12a"}
# filter monocytes_combinationmatrix for combinations covering at least 200kb
monocytes_combmatrix_filt <- monocytes_combinationmatrix[comb_size(monocytes_combinationmatrix) > 100000]
...
```{r q13d_add_distance_to_tss, exercise = TRUE, fig.height=5, exercise.setup = "add_distance_to_tss"}
...
```{r q13e_bivalent_mc, echo = T, eval = F}
